...
import type { LoginRequest, LoginResponse, ChangePasswordResponse } from "./interfaces";

export const loginApi = async (data: LoginRequest): Promise<LoginResponse> => {
  return apiPost<LoginResponse>("/auth/login", data);
};

export const changePasswordApi = async (data: { oldPassword: string; newPassword: string; confirmPassword: string }): Promise<ChangePasswordResponse> => {
...
export const resetPasswordApi = async (data: { email: string; code: string; newPassword: string; confirmPassword: string }) => {
