...
const InputField: React.FC<InputFieldProps> = ({
  ...
  const validateInput = (inputValue: string, validationType?: string): string => {
    ...
    switch (validationType) {
      case 'fullName':
        let cleanedName = inputValue.replace(/[^a-zA-Z\s]/g, '');
      ...
      case 'email':
        let cleanedEmail = inputValue.replace(/\s/g, '');
      ...
      case 'phone':
        let cleanedPhone = inputValue.replace(/[^0-9+]/g, '');
      ...
      case 'passportId':
        let cleanedPassport = inputValue.replace(/[^A-Z0-9]/gi, '').toUpperCase();
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          } py-3 sm:py-4 bg-white/10 border ${
            showValidation && validationType && value.length > 0
              ? isValid 
                ? 'border-green-400/60 focus:ring-green-400' 
                : 'border-red-400/60 focus:ring-red-400'
              : 'border-white/20 focus:ring-blue-400'
          } rounded-2xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:border-transparent backdrop-blur-sm transition-all duration-200 text-sm sm:text-base`}
        ...
        {showValidation && validationType && value.length > 0 && (
          <div className="absolute inset-y-0 right-0 pr-3 sm:pr-4 flex items-center pointer-events-none">
            <div className={`h-4 w-4 sm:h-5 sm:w-5 ${rightIcon ? 'mr-8' : ''}`}>
              {isValid ? (
                <svg className="text-green-400" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/>
                </svg>
              ) : (
                <svg className="text-red-400" fill="currentColor" viewBox="0 0 20 20">
      ...
      {showValidation && validationError && (
        <p className="text-red-400 text-xs sm:text-sm mt-1 text-left animate-fadeIn">
          {validationError}
        </p>
      )}
    </div>
